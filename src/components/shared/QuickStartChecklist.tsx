    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="fixed bottom-4 left-4 w-96 max-w-[calc(100vw-2rem)] bg-white dark:bg-dark-card amoled:bg-amoled-card rounded-2xl shadow-2xl border border-gray-200 dark:border-dark-border amoled:border-amoled-border overflow-hidden"
        >
          {/* Header */}
          <div className="p-6 border-b border-gray-200 dark:border-dark-border amoled:border-amoled-border">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-semibold text-lg text-slate-900 dark:text-dark-text amoled:text-amoled-text">
                    {isAllComplete ? 'Congratulations! 🎉' : 'Quick Start Checklist'}
                  </h3>
                  <p className="text-sm text-gray-600 dark:text-dark-text-secondary amoled:text-amoled-text-secondary">
                    {isAllComplete 
              ...
              <button
                onClick={onClose}
                className="text-gray-400 hover:text-gray-600 dark:hover:text-dark-text amoled:hover:text-amoled-text p-1"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="relative">
              <div className="w-full bg-gray-200 dark:bg-dark-border amoled:bg-amoled-border rounded-full h-3">
                <motion.div
                  initial={{ width: 0 }}
              ...
              <div className="flex justify-between mt-2 text-sm text-gray-600 dark:text-dark-text-secondary amoled:text-amoled-text-secondary">
                <span>{progressPercentage}% complete</span>
                {demoMode && (
                  <span className="bg-green-100 dark:bg-green-900 amoled:bg-green-900 text-green-800 dark:text-green-200 amoled:text-green-200 px-2 py-0.5 rounded text-xs font-medium">
                    Demo Mode
                  </span>
                )}
              </div>
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            {isAllComplete ? (
              ...
              >
                ...
                <h4 className="text-lg font-semibold mb-2 text-slate-900 dark:text-dark-text amoled:text-amoled-text">Amazing Work!</h4>
                ...
                <div className="flex flex-col space-y-2">
                  <div className="flex items-center justify-center text-sm text-green-600 dark:text-green-400 amoled:text-green-400">
                    <CheckCircle className="w-4 h-4 mr-2" />
                    Your job search command center is ready
                  </div>
                  <div className="text-xs text-gray-500 dark:text-dark-text-secondary amoled:text-amoled-text-secondary">
                    ...
                    >
                      <div className="flex-shrink-0 pt-0.5">
                        {task.completed ? (
                          ...
                          >
                            <CheckCircle className="w-5 h-5 text-green-600 dark:text-green-400 amoled:text-green-400" />
                          </motion.div>
                        ) : (
                          <Circle className="w-5 h-5 text-gray-400 dark:text-dark-text-secondary amoled:text-amoled-text-secondary" />
                        )}
                      </div>
          ...
          {/* Footer */}
          {!isAllComplete && (
            <div className="p-4 border-t border-gray-200 dark:border-dark-border amoled:border-amoled-border bg-gray-50 dark:bg-dark-bg/50 amoled:bg-amoled-bg/50">
